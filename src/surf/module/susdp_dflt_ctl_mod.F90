...
RVCOVH2D(:)    = YDVEG%RVCOV(ITVH(:))
RVCOVL2D(:)    = YDVEG%RVCOV(ITVL(:))
RVHSTRH2D(:)   = YDVEG%RVHSTR(ITVH(:))
RVHSTRL2D(:)   = YDVEG%RVHSTR(ITVL(:))
RVLAMSKH2D(:)  = YDVEG%RVLAMSK(ITVH(:))
RVLAMSKL2D(:)  = YDVEG%RVLAMSK(ITVL(:))
RVLAMSKSH2D(:) = YDVEG%RVLAMSKS(ITVH(:))
RVLAMSKSL2D(:) = YDVEG%RVLAMSKS(ITVL(:))
RVRSMINH2D(:)  = YDVEG%RVRSMIN(ITVH(:))
RVRSMINL2D(:)  = YDVEG%RVRSMIN(ITVL(:))
RVZ0MH2D(:)    = YDVEG%RVZ0M(ITVH(:))
RVZ0ML2D(:)    = YDVEG%RVZ0M(ITVL(:))
RVZ0HH2D(:)    = YDVEG%RVZ0H(ITVH(:))
RVZ0HL2D(:)    = YDVEG%RVZ0H(ITVL(:))
RVRSMINB2D(:)  = 50.0_JPRB

DO JK=1, KLEVS3D
  RVROOTSAH3D(:,JK) = YDVEG%RVROOTSA(JK, ITVH(:))
  RVROOTSAL3D(:,JK) = YDVEG%RVROOTSA(JK, ITVL(:))
ENDDO
