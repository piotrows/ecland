...
REAL(KIND=JPRB)   :: ZLEVMAX(KLON,KLEVSN),ZLEVMINA(KLON,KLEVSN)
REAL(KIND=JPRB)   :: ZDSNTOT, ZDSN(KLON,KLEVSN)
REAL(KIND=JPRB)   :: ZTMP0,ZTMP1,ZEPSILON, KLMAXVAL

REAL(KIND=JPRB)   :: ZRLEVSNMAX(KLON,KLEVSN), ZRLEVSNMIN(KLON,KLEVSN)
...
DO JL=KIDIA,KFDIA
   ! layers snow depth 
  ZDSN(JL,1:KLEVSN) = PSSN(JL,1:KLEVSN) / PRSN(JL,1:KLEVSN) 
   ! total snow depth 
  ZDSNTOT = SUM(ZDSN(JL,1:KLEVSN))
  IF (LDLAND(JL))THEN
  IF (PCIL(JL) >= 0.99_JPRB) THEN ! fully resolved glacier
      ZRLEVSNMIN(JL,1:KLEVSN) = RLEVSNMIN_GL(1:KLEVSN)
      ZRLEVSNMAX(JL,1:KLEVSN) = RLEVSNMAX_GL(1:KLEVSN)
    ELSEIF  (ZDSNTOT>ZDSNTHR ) THEN
      ...
      ! we do not change discretization over flat terrain
        ZRLEVSNMIN(JL,1:KLEVSN) = RLEVSNMIN(1:KLEVSN) 
        ZRLEVSNMAX(JL,1:KLEVSN) = RLEVSNMAX(1:KLEVSN) 
      ELSE  
        ZVAR_LEVSNMIN        = 0.10_JPRB+ZALFA_G*ZDSNDIFF
        ZRLEVSNMIN(JL,1:KLEVSN) = MIN(0.25_JPRB,ZVAR_LEVSNMIN)

        ZVAR_LEVSNMAX        = 0.15_JPRB+ZALFA_G*ZDSNDIFF
        ZRLEVSNMAX(JL,1:KLEVSN) = ZVAR_LEVSNMAX
        ZRLEVSNMAX(JL,1)        = ZRLEVSNMIN(JL,1)
        ZRLEVSNMAX(JL,1:KLEVSN) = MIN(0.30_JPRB, ZRLEVSNMAX(JL,1:KLEVSN))
        ZRLEVSNMAX(JL,KLEVSN)   = -1._JPRB ! accumulation layer
      ENDIF
    ELSE 
      ZRLEVSNMAX(JL,1:KLEVSN) = RLEVSNMAX(1:KLEVSN) 
      ZRLEVSNMIN(JL,1:KLEVSN) = RLEVSNMIN(1:KLEVSN) 
  ...
  KLMAXVAL =ZRLEVSNMAX(JL,1)
  KLMAX = 1
  DO JK=1,KLEVSN
    IF (ZRLEVSNMAX(JL,JK) < KLMAXVAL) THEN
      KLMAXVAL = ZRLEVSNMAX(JL,JK)
      KLMAX = JK
    ENDIF
  ENDDO

  ZLEVMAX(JL,:) = ZRLEVSNMAX(JL,:)     
  IF ( ZRLEVSNMAX(JL,KLMAX) > 0._JPRB ) THEN
    CALL ABORT_SURF("SRFSN_VGRID: RLEVSNMAX(KLMAX) should be < 0")
  ENDIF
  ZLEVMAX(JL,KLMAX) = 1.E10_JPRB 
  
! accumulated min snow depth 
  ZLEVMINA(JL,1) = ZRLEVSNMIN(JL,1)
  DO JK=2,KLEVSN
    ZLEVMINA(JL,JK) = ZLEVMINA(JL,JK-1) +  ZRLEVSNMIN(JL,JK)
  ENDDO
...
! 1. Find # of active snow layer
  ...
  ELSE
    ILEVSNA(JL) = 1
    DO JK=2,KLEVSN
      IF ( ZDSNTOT < ZLEVMINA(JL,JK) ) THEN
        ILEVSNA(JL) = JK-1
        EXIT
      ELSE
        ILEVSNA(JL) = JK 
...
! 2. Apply the layering
  ...
  ELSE
    IF (ILEVSNA(JL) > 1 ) THEN
      PDSNOUT(JL,1) = MIN(ZDSNTOT,ZLEVMAX(JL,1))
      ZTMP0= (ZDSNTOT-PDSNOUT(JL,1)) / (ILEVSNA(JL) - 1.0)
      ! 1st screen
      ZTMP1=PDSNOUT(JL,1)
      DO JK=2,ILEVSNA(JL)
        PDSNOUT(JL,JK) = MIN(ZTMP0,ZLEVMAX(JL,JK))
