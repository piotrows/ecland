...
REAL(KIND=JPRB)    :: ZSNOTRSTMP(KLON,KLEVSN+1)
...
DO JT=1,KSNTILES
  ...
  !! Preparation
      ...
      ENDDO
      ZSNOTRSTMP(JL,1:KLEVSN+1)=0._JPRB
      ZSNEXTCOEFF(JL,1:KLEVSN)=25._JPRB
      ZSNQRAD=0._JPRB
  
        DO JK=1, KLACT
          ZDSN(JL,JK)=MAX(0._JPRB, MIN(RDSNMAX,PSSNM1M(JL,JK)/(ZFRSN_TOT*PRSNM1M(JL,JK))))
        ...
        ENDDO
        ZSNSOABS(JL)=MAX(ZEPSILON, exp(-ZSNEXTCOEFF(JL,1)*ZDSN(JL,1)) )
        ZSNOTRSTMP(JL,1)= PSSRFLTI(JL,JTILE)*ZSNSOABS(JL)
        ZSNQRAD=PSSRFLTI(JL,JTILE)-PSSRFLTI(JL,JTILE)*(1._JPRB-ZSNSOABS(JL))
        IF (KLACT > 1) THEN
          DO JK=2, KLACT
            ZSNSOABS(JL)=exp(-ZSNEXTCOEFF(JL,JK)*ZDSN(JL,JK))
            ZSNOTRSTMP(JL,JK)=ZSNQRAD*(1._JPRB-ZSNSOABS(JL))
            ZSNQRAD=ZSNQRAD-ZSNOTRSTMP(JL,JK)
          ENDDO
        ENDIF
